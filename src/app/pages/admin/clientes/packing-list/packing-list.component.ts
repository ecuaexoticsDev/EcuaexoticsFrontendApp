...
export class PackingListComponent implements OnInit {
  ...
  public cajasCat: any[] = []; // cajas de 14,5kg
  ...
  ngOnInit(): void {
    this.rutaActiva.params.subscribe(({ id, idPack }) => {
            ...
            element.items_tipo_pitahaya.forEach((items: any) => {
              items.items_tipo_caja.forEach((tipoCaja: any) => {
                if (tipoCaja.tipo_caja === "Carton Box 2.5 kg net weight") {
                  ...
                  tipoCaja.tipo_caja === "Carton Box 4 kg net weight"
                ) {
                  tipoCaja.calibres.forEach((data: any) => {
                    this.cajasCuatro.push(data);
                  });
                } else if (
                  tipoCaja.tipo_caja === "Carton Box 4.5 kg net weight"
                ) {
                  tipoCaja.calibres.forEach((data: any) => {
                    this.cajasGrandes.push(data);
                  });
                } else if (
                  tipoCaja.tipo_caja === "Carton Box 10.9 kg net weight"
                ) {
                  tipoCaja.calibres.forEach((data: any) => {
                    this.cajasDiez.push(data);
                  });
                } else if (
                  tipoCaja.tipo_caja === "Carton Box 12.7 kg net weight"
                ) {
                  tipoCaja.calibres.forEach((data: any) => {
                    this.cajasDoce.push(data);
                  });
                } else if (
                  tipoCaja.tipo_caja === "Carton Box 14.5 kg net weight"
                ) {
                  tipoCaja.calibres.forEach((data: any) => {
                    this.cajasCat.push(data);
                  });
                }
              });
            });
  ...
  submitForm(): void {
    ...
    this.clientesService
      .updatePackingList(this.id_packing, this.packing)
      .subscribe(
        (resp) => {
        ...
        (error) => {
          Swal.fire(
            "Error",
            "Sucedio un error Inersperado, intentelo mas tarde.",
            "error"
          );
        }
      );
