...
export class InvoceComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.rutaActiva.params.subscribe(({ id, idInv }) => {
      this.getCliente(id);
        ...
        resp.forEach((element: any) => {
          if (this.id_factura == element.factura.id_factura) {
            ...
            if (this.factura.fecha !== null) {
              this.invoceForm
                .get("fecha")
                ?.setValue(new Date(this.factura.fecha));
            }
  ...
  saveEdit(data: any): void {
    ...
    const index = this.listOfData.findIndex(
      (item) => item.id_item_factura === data.id_item_factura
    );
    this.clientesService
      .updatePrecio(data.id_item_factura, itemUpdate.precio_caja)
      .subscribe(
        (resp: any) => {
          Object.assign(
            this.listOfData[index],
            this.editCache[data.id_item_factura].data
          );
        ...
        (err) => {
          ...
          Swal.fire(
            "Error",
            "Sucedio un error, no se pudo actualizar el Precio",
            "error"
          );
          this.editCache[data.id_item_factura].edit = false;
        }
