...
export class ConsolidadoComponent implements OnInit {
  ...
  cargarProductores() {
    this.productoresService.cargarProductores().subscribe(
      ...
      (err) => {
        Swal.fire(
          "Error",
          "Sucedio un error, no se pudo cargar los Productores",
          "error"
        );
      }
    );
   ...
   */
  cargarTransportes() {
    this.transportesService.getCamiones().subscribe((resp: any) => {
  ...
  cargarConsolidado(
    ...
    this.consolidadoService
      .cargarConsolidado(fechas, lote!, id_prod!, fruta!, unidad!)
      .subscribe(
        (resp: consolidado[]) => {
          this.Procesos = resp;
        ...
        (err) => {
          Swal.fire(
            "Error",
            "Sucedio un error, no se pudo cargar los Registros",
            "error"
          );
        }
      );
