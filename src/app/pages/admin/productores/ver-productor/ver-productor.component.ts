...
export class VerProductorComponent implements OnInit {
  ...
  getProductor(id_productor: number) {
    this.cargando = true;
    this.productoresService.getProductorByid(id_productor).subscribe(
      (productor: any) => {
      ...
      (err) => {
        Swal.fire(
          "Error",
          "Sucedio un error, no se pudo cargar el Productor",
          "error"
        );
      }
    );
  ...
  cargarBitacoras(id_productor: number) {
    this.bodegaExternaService.obtenerGavetasProductor(id_productor).subscribe(
      (resp: any) => {
        this.bitacoras = resp;
      },
      (err) => {
        Swal.fire(
          "Error",
          "Sucedio un error, no se pudo cargar las Bitacoras",
          "error"
        );
      }
    );
  ...
  cargarLiquidaciones(id_productor: number) {
    this.productoresService.obtenerLiquidaciones(id_productor).subscribe(
      (resp: any) => {
      ...
      (err) => {
        Swal.fire(
          "Error",
          "Sucedio un error, no se pudo cargar las Liquidaciones",
          "error"
        );
      }
    );
   ...
   * @param {Date[]} result filtro de las fechas seleccionadas por el usuario
   */
  onChange(result: Date[]) {
    ...
    if (result.length !== 0) {
      this.liquidaciones.forEach((element) => {
        ...
        if (
          fechaLiq.getTime() >= this.rangofechas[0].getTime() &&
          fechaLiq.getTime() <= this.rangofechas[1].getTime()
        ) {
  ...
  onChangeBit(result: Date[]) {
    ...
    if (result.length !== 0) {
      this.bitacoras.forEach((element) => {
        ...
        if (
          fechabit.getTime() >= this.rangofechasBit[0].getTime() &&
          fechabit.getTime() <= this.rangofechasBit[1].getTime()
        ) {
   ...
   * @param {number} id id del reporte para descargarlo
   */
   ...
   * @param {number} id id de la bitacora para descargarla
   */
          ...
          content: [
            {
              stack: [
                ...
                reportBitacora.DetailControl(
                  bitacora,
                  productor,
                  controlCalidad.gav_vacias,
                  controlCalidad.num_gav_rechazo
                ),
              ],
              margin: [0, 0, 0, 0],
            },
